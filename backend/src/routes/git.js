...
router.get('/status', async (req, res) => {
  try {
    ...
    if (project.isLite) {
      const status = await githubService.getLiteStatus(project.repoUrl, project.lastBranch);
...
router.get('/branches', async (req, res) => {
  try {
    const project = resolveProject(req);
    if (project.isLite) {
      const branches = await githubService.getLiteBranches(project.repoUrl, project.lastBranch);
...
router.post('/branches/switch', async (req, res) => {
  try {
    ...
    if (project.isLite) {
      // For lite projects, we just update the "current view" branch in the store
      const updated = store.updateProject(project.id, { lastBranch: name });
      return res.json({ success: true, branch: name, isLite: true });
    }
...
router.get('/log', async (req, res) => {
  try {
    ...
    if (project.isLite) {
      const log = await githubService.getLiteLog(project.repoUrl, { 
        limit: parseInt(limit) || 50, 
        branch: branch || project.lastBranch 
      });
      return res.json(log);
    }
